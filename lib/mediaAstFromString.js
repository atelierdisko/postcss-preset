...
function parse(string, splitByAnd) {
  ...
  while (++i < string.length) {
    ...
    if (split) {
      array.push(
        splitByAnd ? new MediaExpression(buffer + char) : new MediaQuery(buffer)
      );
  ...
  if (buffer !== "") {
    array.push(
      splitByAnd ? new MediaExpression(buffer) : new MediaQuery(buffer)
    );
  }
...
class MediaQuery {
  ...
  toString() {
    ...
    return `${raws.before}${this.modifier}${
      this.modifier ? `${raws.afterModifier}` : ""
    }${this.type}${raws.beforeAnd}${raws.and}${
      raws.beforeExpression
    }${this.nodes.join("")}${this.raws.after}`;
  }
}

class MediaExpression {
  constructor(string) {
    const [, value, after = "", and = "", afterAnd = ""] = string.match(
      andRegExp
    ) || [null, string];
...
const spaceWrapRegExp = new RegExp(
  `^${noSpaceRE}${noExpressionRE}${noSpaceRE}$`
);
const mediaRegExp = new RegExp(
  `^(?:${modifierRE}${spaceRE})?(?:${typeRE}(?:${andRE}${spaceRE}${expressionRE})?|${expressionRE})$`,
  "i"
);

module.exports = (string) => new MediaQueryList(string);
